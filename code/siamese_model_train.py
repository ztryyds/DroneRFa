...
# 构建和编译模型
model = build_siamese_model(input_shape, num_classes)
...
history = model.fit(
    ...
    validation_data=([data_test, data_test], label_test_encoded),
    epochs=100,
    batch_size=32,
    callbacks=[
        ModelCheckpoint('../model/best_siamese_model.h5', save_best_only=True, verbose=1),
...
plt.ylabel('Loss')
plt.legend()
plt.savefig('../img/siamese.png')  # 保存图像
plt.close()
