...
def weibull_tailfitting(meanfiles_path, distancefiles_path, labellist, tail_percent, distance_type):
    weibull_model = {}
...
def main():
    ...
    euclidean_model = weibull_tailfitting(mean_vector_path, distance_save_path, label_allow_list,
                                          tail_percent=0.02, distance_type='euclidean')
    cosine_model = weibull_tailfitting(mean_vector_path, distance_save_path, label_allow_list,
                                       tail_percent=0.02, distance_type='cosine')
    eucos_model = weibull_tailfitting(mean_vector_path, distance_save_path, label_allow_list,
                                      tail_percent=0.02, distance_type='eucos')
    with open('../model/euclidean_model1.pkl', 'wb') as f:
        pickle.dump(euclidean_model, f)
        print('euclidean_model has saved')

    with open('../model/cosine_model1.pkl', 'wb') as f:
        pickle.dump(cosine_model, f)
        print('cosine_model has saved')

    with open('../model/eucos_model1.pkl', 'wb') as f:
