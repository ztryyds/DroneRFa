...
class DataGenerator_pair(tf.keras.utils.Sequence):
    def __init__(self, file_paths, labels, batch_size=32, dim=(256, 236, 4), shuffle=True):
    ...
    def __data_generation(self, file_paths_batch, labels_batch,real_batch_size):
        # 初始化数据和标签数组
        X1 = np.empty((real_batch_size, *self.dim),dtype=np.float32)
        X2 = np.empty((real_batch_size, *self.dim),dtype=np.float32)
        ...
        for i, file_pair in enumerate(file_paths_batch):
            ...
            with h5py.File(file_path1, 'r') as file:
                X1[i,] = file['STFT Magnitude'][:]
                y[i] = labels_batch[i]
            with h5py.File(file_path2, 'r') as file:
                X2[i,] = file['STFT Magnitude'][:]
        return X1,X2,y
