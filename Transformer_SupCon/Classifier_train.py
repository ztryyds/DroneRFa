...
max_epochs = config.getint('general', 'ce_epochs')
...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.Adam(Classifier_model.parameters(), lr=max_lr)
    cosine_scheduler = CosineAnnealingLR(optimizer, T_max=max_epochs, eta_min=min_lr)
    # 设置日志保存
    csv_file = open(config.get('general', 'path_to_ce_log'), 'w', newline='')
    fieldnames = ['epoch', 'train_loss', 'train_accuracy', 'val_loss', 'val_accuracy', 'epoch_time', 'lr']
    ...
    for epoch in range(max_epochs):
        ...
        for batch_idx, (data, targets) in enumerate(train_loader):
            ...
            correct_tensor = torch.as_tensor(predicted == targets, dtype=torch.int64)
        ...
        with torch.no_grad():
            for data, targets in val_loader:
                ...
                correct_tensor = torch.as_tensor(predicted == targets, dtype=torch.int64)
                val_correct += correct_tensor.sum().item()
                val_loss += criterion(logits, targets).item() * targets.size(0)
        end_time = time.time()
        ...
        print(
            f"Epoch {epoch}, Train Loss: {train_loss}, Train Accuracy: {train_accuracy}， Val Loss: {val_loss}, "
            f"Val Accuracy: {val_accuracy}, epoch_time: {epoch_time}, lr: {lr}")

        try:
            writer_csv.writerow(
                {'epoch': epoch, 'train_loss': train_loss, 'train_accuracy': train_accuracy, 'val_loss': val_loss,
                 'val_accuracy': val_accuracy, 'epoch_time': epoch_time, 'lr': lr})
            csv_file.flush()  # 刷新文件缓冲区，确保数据立即写入文件
        except Exception as e:
            print(f"Error writing to CSV: {e}")

        # 保存模型
        torch.save(Classifier_model.state_dict(), config.get('general', 'path_to_ce'))
        print(f"model has improved")
